...
|---|---|---|---|
| `agent-{name}` | Private | MEMORY.md + daily memory files, plus your outputs, research docs, and workspace files | **Auto** for memory (re-indexed every heartbeat); **agent-managed** for outputs |
| `shared-{org}` | Org-wide | Research findings, reports, org knowledge | **Agent-managed** — ingest when the whole org benefits |

**Memory indexing into agent-{name} is automatic.** On every heartbeat cycle, re-ingest your memory files so they stay current and searchable:
```bash
...
- To recall your own past work: query with `--scope private` (your `agent-{name}` collection)

**When to ingest to agent-{name} (--scope private) and shared-{org} (--scope shared) — your judgment:**
- After completing a task with a notable output → `--scope private` (agent-{name})
...
- After the user shares a file with you → `--scope private` (agent-{name})
